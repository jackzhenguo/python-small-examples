...
目前已发布[Python之路.pdf V1.0](https://github.com/jackzhenguo/python-small-examples/releases)，包括：`Python之基`，`Python之正`，`Python之例`，`Python之能`章节，共计`143个`小例子。欢迎关注《Python小例子》官方公众号：

![](./img/python-small-examples.png)

### 一、Python之基

`Python之基`主要总结Python常用内置函数及用法，它们在Python中被最高频的使用，所以务必掌握。`V1.0` 一共包括`59个`。
...
`Python之例`章中每个例子大都10行左右，1.0版本一共包括`36个小例子`，都是很有意思的小例子。
